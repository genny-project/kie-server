...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="//ds:subsystem/ds:datasources/ds:datasource[@jndi-name='java:jboss/datasources/ExampleDS']">
        <ds:datasource jndi-name="java:jboss/datasources/jbpmDS" pool-name="jbpmDS" enabled="true" use-java-context="true" use-ccm="false">
            <ds:connection-url>jdbc:mysql:loadbalance://${env.MYSQL_PORT_3306_TCP_ADDR:db}:${env.MYSQL_PORT_3306_TCP_PORT:3306}/${env.MYSQL_DATABASE:db}?failOverReadOnly=false&amp;maxReconnects=100&amp;zeroDateTimeBehavior=convertToNull&amp;autoReconnect=true&amp;characterEncoding=UTF-8&amp;characterSetResults=UTF-8&amp;useUnicode=true&amp;interactiveClient=true&amp;connectTimeout=60000&amp;socketTimeout=60000</ds:connection-url>
            <ds:driver>mysql</ds:driver>
            <ds:security>
                <ds:user-name>${env.MYSQL_USER:admin}</ds:user-name>
                <ds:password>${env.MYSQL_PASSWORD:admin}</ds:password>
            </ds:security>
            <ds:validation>
